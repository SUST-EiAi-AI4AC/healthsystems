...
print("\nStep 2: Uploading JAR to remote server via Optimized SFTP (Pipelined)...")
try:
    transport = paramiko.Transport((hostname, 22))
    # Increase window size for faster transfer
    ...
    with open(local_jar, 'rb') as local_file:
        with sftp.open(remote_jar, 'wb') as remote_file:
            # Enable pipelining for parallel requests
            remote_file.set_pipelining(True)
            ...
            while True:
                ...
                sys.stdout.write(f"\rTransferred: {transferred}/{file_size} bytes ({percent:.2f}%) | Speed: {speed:.2f} MB/s")
                sys.stdout.flush()
                
    print(f"\nSFTP Upload completed successfully in {time.time() - start_time:.1f} seconds!")
    sftp.close()
    transport.close()
