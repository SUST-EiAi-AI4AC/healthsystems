import paramiko

hostname = "47.109.49.174"
username = "root"
password = "nwpuhs@ABC123!@#"

print("Connecting to remote server...")
try:
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(hostname, username=username, password=password, timeout=15)
    print("Connected successfully!")
    
    # Run the fix, drop & recreate, and import
    cmds = [
        # Replace utf8mb4_0900_ai_ci with utf8mb4_general_ci in the SQL file
        "sed -i 's/utf8mb4_0900_ai_ci/utf8mb4_general_ci/g' /root/healthsystem_*2025*.sql",
        
        # Drop and recreate database for clean import
        "mysql -u root -pnwpuhs@ABC123\\!@# -e \"DROP DATABASE IF EXISTS healthsystem_test2; CREATE DATABASE healthsystem_test2 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;\" || true",
        
        # Import the updated SQL file
        "mysql -u root -pnwpuhs@ABC123\\!@# healthsystem_test2 < /root/healthsystem_*2025*.sql",
        
        # Verify the list of tables
        "mysql -u root -pnwpuhs@ABC123\\!@# -e \"USE healthsystem_test2; SHOW TABLES;\" || true",
        
        # Check user table count
        "mysql -u root -pnwpuhs@ABC123\\!@# -e \"USE healthsystem_test2; SELECT COUNT(*) FROM user;\" || true",
        
        # Restart backend service
        "systemctl restart healthsystem || true",
        "sleep 5",
        "systemctl status healthsystem || true"
    ]
    
    for cmd in cmds:
        stdin, stdout, stderr = ssh.exec_command(cmd)
        print(f"\n==================================================")
        print(f"Executing: {cmd}")
        print(f"==================================================")
        out = stdout.read().decode('utf-8', errors='ignore').strip()
        err = stderr.read().decode('utf-8', errors='ignore').strip()
        if out:
            print("STDOUT:\n", out)
        if err:
            print("STDERR:\n", err)
        
    ssh.close()
except Exception as e:
    print(f"Error: {e}")
